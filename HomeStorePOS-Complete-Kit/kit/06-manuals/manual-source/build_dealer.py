...
class Doc(BaseDocTemplate):
    def __init__(self,fn,**kw):
        BaseDocTemplate.__init__(self,fn,pagesize=letter,leftMargin=0.85*inch,rightMargin=0.85*inch,topMargin=0.9*inch,bottomMargin=0.85*inch,title="MyCoreRetail — Store User Guide",author="MyCoreRetail",**kw)
    ...
    def main_page(self,canv,doc):
        ...
        canv.setFont(BASE,8.5); canv.setFillColor(INK2); canv.drawString(doc.leftMargin,H-0.55*inch,"MyCoreRetail — Store User Guide"); canv.drawRightString(W-doc.rightMargin,H-0.55*inch,self.chapter)
...
cov=Table([[Paragraph("MyCoreRetail",st["cover1"])],[Paragraph("Store User Guide — for managers, sales staff, warehouse and delivery crews",st["cover2"])],[Spacer(1,18)],[Paragraph("Point of sale, inventory, purchasing, fulfillment, delivery routing, warehouse, service and reporting for furniture, mattress and home-goods retailers.",st["cover2"])],[Spacer(1,40)],[Paragraph("Edition: %s"%datetime.date.today().strftime("%B %Y"),st["cover3"])],[Paragraph("Covers the store application: selling, inventory, purchasing, fulfillment, deliveries, warehouse, service and reports.",st["cover3"])]],colWidths=[W-2*inch])
...
H2("Your store, your data"),
P("Each store runs as its own completely separate system with its own staff, inventory, customers, sales, settings and look. Staff sign in with a name and PIN on any register, tablet or phone that has opened the store link."),
...
H2("2.1 Opening your store"),
steps(["Open the store link you were given (it looks like …#store/NFM01) on each register, tablet and phone. The device remembers your store from then on.","Tap your name and enter your PIN. The first manager login is set up when the store is created; managers add everyone else under Settings → Staff &amp; permissions.","Bookmark the page or add it to the home screen on phones and tablets used by the crew."]),
H2("2.2 First-day checklist"),
bullets(["Settings → Store &amp; tax: store name, phone, address and map pin, tax brackets, logo and colors.","Settings → Delivery &amp; routing: trucks, delivery days, capacity, delivery price matrix.","Settings → Pricing &amp; financing: supplier cost rules, payment methods, lease/finance companies, protection plan rules.","Settings → Staff &amp; permissions: add staff with PINs, roles and limits.","Inventory: upload your items from a spreadsheet (chapter 4.6) or add them one by one; enter suppliers.","Settings → Optional features: turn on only what the store uses."]),
H2("2.3 Registers, devices and switching users"),
bullets(["A device that has opened the store link shows only this store's sign-in.","Tap your name, enter your PIN. Managers add staff under Settings → Staff &amp; permissions.","<b>Idle lock</b>: the register locks after a few minutes without a touch (Settings → Pricing → Shared register security). An unfinished sale is restored when someone signs back in.","<b>Switch user</b>: account menu (⋯ at the bottom of the sidebar) → Lock register / switch user.","The account menu also holds Clock in / out (when the Time clock feature is on) and dark mode."]),
H2("2.4 Branding"),
...
shot("22_features.png","Optional features — switch on what the store uses."),
H2("3.7 Data &amp; backup"),
P("Export a full backup of the store, import one, or reset sample data."),
PageBreak()]
...
H2("4.3 Suppliers"),
P("Inventory → Suppliers: contact, account #, lead time, payment terms, minimum order, and the supplier's own cost rules. Suppliers can carry a catalog feed (JSON/CSV) and, when MyCoreRetail has connected that vendor, a shared catalog you pull from at your own markup."),
...
P("A supplier can import a JSON/CSV catalog feed with field mapping. When MyCoreRetail has connected a vendor, it appears under Suppliers: connect it at your own multiplier, pull the whole catalog or browse and add single items, and get “catalog updated” prompts with cost changes. Purchase orders to that supplier can be sent in the vendor's order format."),
PageBreak()]
...
S=flat
doc=Doc("/mnt/user-data/outputs/MyCoreRetail-Store-User-Guide.pdf")
doc.multiBuild(S)
print("built")
